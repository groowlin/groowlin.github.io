...
- Client route swaps keep the card backing and right navigation arrow static. Route-content snapshots form a continuous bidirectional vertical wheel/tape inside one clipped viewport. Every snapshot has a unique instance on one shared track offset.
- Direction is derived from the latest requested target, including the optimistic click target: `/ → non-home` moves upward by `-100%`, `non-home → /` moves downward by `+100%`, and `non-home → non-home` moves upward by `-100%`. A matching snapshot already located in the requested direction is reused, with the nearest match preferred, so a fast `/ ↔ non-home` toggle reverses the current wheel instead of accumulating duplicate snapshots.
- A new click during motion does not wait for completion. The current track tween stops and retargets the same motion value from its current rendered offset without snapping. Each retarget gets a new `runId`: stale completions are ignored, and snapshot cleanup runs only after the current `runId` completes.
- Every track retarget uses `motion-emphasis` (`420ms`) with the standard easing (`cubic-bezier(0.4, 0, 0.2, 1)`). Same-direction burst targets may extend the remaining distance and therefore visibly accelerate; a direction reversal preserves the current rendered position.
- Snapshot scale is symmetric by track position: `1 / 1.5` (`≈0.667`) at `+100%`, `1` at center `0%`, and `1 / 1.5` at `-100%`. Scale transform origin is the center of the complete card viewport (`50% 50%`), not the intrinsic bounds of its visible content. Opacity is also derived only from track position: below center it changes `0 → 1` from `+100% → +36%`, and above center it changes `1 → 0` from `0% → -64%`. The same curves therefore run inversely and continuously when the track reverses. Route-swap blur remains `0`.
